...
string Bitsetostring_de(bitset<64> bits)
{
	char s[8];
	int c;
	int two;
...
bitset<64> decrypt(bitset<64>& plain)
{
	bitset<64> cipher;
	bitset<64> current;
	bitset<32> left;
...
int main() {
	...
	ifstream plaintext("DES-Key-Plaintext.txt");
	ofstream out("out.txt");
	s.resize(8), k.resize(8);
	for(int i = 0; i < 5; i++)
	{
		for(int i=0; i<8; i++)
		{
			k.at(i) = plaintext.get();//12345678 Advanced
		}
		plaintext.get();
		for(int i=0; i<8; i++)
		{
			s.at(i) = plaintext.get();
		}
		plaintext.get();
		space = plaintext.get();
		...
		//bitset<64> de = decrypt(cipher);
		out << k << "-" << s << "-" << bitsetToString(cipher) << "\n";
		//cout << Bitsetostring_de(de) << "\n";
		for(int i = 0; i < 199; i++)
		{
			bitset<64> plain = charToBitset(s.c_str());
			bitset<64> key = charToBitset(k.c_str());
